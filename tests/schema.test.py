...
# --- v10: one load hierarchy for the whole dashboard ------------------------
# The same wiring fact was stored twice — once per card — so declaring it in the
# economy card left the flow diagram drawing the two loads in parallel.
old_h = {"version": 9, "revision": 2, "pages": [
    {"id": "p", "type": "sections", "title": "Casa", "sections": [
        {"id": "s", "title": "Energia", "items": [
            {"id": "eco", "type": "economy", "devices": [
                {"entity": "sensor.presa", "name": "Presa"},
                {"entity": "sensor.friggitrice", "name": "Friggitrice", "parent": "sensor.presa"}]},
            {"id": "flow", "type": "energyflow", "flow": {"devices": [
...
messy = schema.normalize_dashboard({"version": 10, "hierarchy": {
    "sensor.a": "sensor.b", "sensor.c": "senzapunto", "nonvalido": "sensor.b",
    "sensor.d": "sensor.d", "sensor.e": 7}})
assert messy["hierarchy"] == {"sensor.a": "sensor.b"}, messy["hierarchy"]
# A cycle would make every total uncomputable and hang a renderer walking up.
# One link is removed, not the whole map: the rest of what the user declared
# is still valid and must survive. The invariant is that nothing loops.
cyc = schema.normalize_dashboard({"version": 10, "hierarchy": {
    "sensor.a": "sensor.b", "sensor.b": "sensor.c", "sensor.c": "sensor.a"}})


def loops(h):
    for start in h:
        seen, node = {start}, h.get(start)
        while node:
            if node in seen:
                return True
            seen.add(node)
...
assert not loops(cyc["hierarchy"]), cyc["hierarchy"]
assert 0 < len(cyc["hierarchy"]) < 3, cyc["hierarchy"]
assert not loops(schema.normalize_dashboard(cyc)["hierarchy"])
# and a legitimate chain survives untouched
chain = schema.normalize_dashboard({"version": 10, "hierarchy": {
    "sensor.a": "sensor.b", "sensor.b": "sensor.c"}})
assert chain["hierarchy"] == {"sensor.a": "sensor.b", "sensor.b": "sensor.c"}
